...
def calculate_magnified_moment(Pu, M_demand, width, depth, fc, lu_mm, k, cm_factor, beta_dns):
    ...
    if Pu <= 0:
        return pd.Series([abs(M_demand), 1.0])
    ...
    Pc_newtons = (math.pi ** 2 * EI) / (k * lu_mm) ** 2
    Pc = Pc_newtons / 1000

    if Pu >= 0.75 * Pc:
        return pd.Series([9999.9, 999.9])
...
if uploaded_file is not None:
    ...
    if 'Story' in df_raw.columns and 'Column' in df_raw.columns:
        ...
        df['P_Demand_kN'] = df['P'] * -1  # Flip sign so compression is positive
    ...
    # --- Sidebar: Slenderness Parameters ---
    ...
    if auto_optimize:
        ...
        if best_layout:
            ...
            st.success(f"✅ **Optimal Layout Found:** {optimized_name} ($A_{{st}} = {round(ast, 1)}$ $mm^2$)")
        else:
            ast = b * h * 0.08
            st.error("❌ **Optimization Failed:** Section fails even at 8% reinforcement limit.")
    else:
    ...
    display_columns = ['Load_Combo', 'P_Demand_kN', 'M2_Demand_kNm', 'Delta_2', 'M2_Mag_kNm', 'M3_Demand_kNm',
                       'Delta_3', 'M3_Mag_kNm']
    styled_df = df[display_columns].style.map(highlight_high_delta, subset=['Delta_2', 'Delta_3'])
